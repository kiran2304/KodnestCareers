...
const JobService = {
    ...
    init: () => {
        if (!localStorage.getItem('jobsy_jobs')) {
            localStorage.setItem('jobsy_jobs', JSON.stringify(INITIAL_JOBS));
            localStorage.setItem('jobsy_last_sync', new Date().toISOString());
        }
    },

    // Get all jobs (sorted by newest first)
    getAll: () => {
        const jobs = JSON.parse(localStorage.getItem('jobsy_jobs') || '[]');
    ...
    // Get Last Sync Time
    getLastSync: () => {
        return localStorage.getItem('jobsy_last_sync');
    },
    ...
    sync: () => {
        ...
        const existingJobs = JSON.parse(localStorage.getItem('jobsy_jobs') || '[]');
        // Prepend new jobs
        const updatedJobs = [...newJobs, ...existingJobs];

        // Keep list size manageable (max 50)
        if (updatedJobs.length > 50) {
            updatedJobs.length = 50;
        }

        localStorage.setItem('jobsy_jobs', JSON.stringify(updatedJobs));
        localStorage.setItem('jobsy_last_sync', new Date().toISOString());
